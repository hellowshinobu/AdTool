...
static bool
CopySections(const unsigned char *data, size_t nDataSize, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module)
{
	int i, size;
	...
	for (i = 0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
		...
		if (section->PointerToRawData + section->SizeOfRawData > nDataSize){
			return false;
		}
...
HMEMORYMODULE MemoryLoadLibrary(const void *data,size_t n)
{
	return MemoryLoadLibraryEx(data, n, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL);
}

HMEMORYMODULE MemoryLoadLibraryEx(const void *data,size_t n,
	CustomLoadLibraryFunc loadLibrary,
	CustomGetProcAddressFunc getProcAddress,
	CustomFreeLibraryFunc freeLibrary,
	void *userdata)
{
	...
	//检测Imge大小与载入数据大小
// 	if (old_header->OptionalHeader.SizeOfImage > n){
...
// 	}
	...
	if (CopySections(static_cast<const unsigned char *>(data) , n, old_header, result) == false){
		SetLastError(ERROR_FILE_CORRUPT);
		return FALSE;
	}
	...
	__HCODE__
		...
		while (end > start) {
			...
			while (end > start) {
				...
				size_t nCvt;
				wcstombs_s(&nCvt, searchKey, resourceString->Length, resourceString->NameString, resourceString->Length);
...
MemoryLoadStringEx(HMEMORYMODULE module, UINT id, LPTSTR buffer, int maxsize, WORD language)
{
	...
	data = static_cast<PIMAGE_RESOURCE_DIR_STRING_U>(MemoryLoadResource(module, resource));
	id = id & 0x0f;
	while (id--) {
...
#else
	size_t nCvt;
	wcstombs_s(&nCvt, buffer, maxsize, data->NameString, size);
#endif
	return size;
}
